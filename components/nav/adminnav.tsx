...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  return (
    <Box
      ...
      {LinkItems.map((link) => (
        <NavItem location={link.location} key={link.name} icon={link.icon}>
          {link.name}
...
const NavItem = ({ icon, children, location, ...rest }: NavItemProps) => {
  return (
    <Link href={location} style={{ textDecoration: 'none' }} _focus={{ boxShadow: 'none' }}>
      <Flex
        align="center"
        p="4"
        mx="4"
